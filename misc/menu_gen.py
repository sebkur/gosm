...
def print_item(label, path, has_subs, part):
	...
	if part == 1:
		...
		text_menu += fill2 * ' '
		if len(label) <= 2:
			text_item += "= gtk_menu_item_new_with_label(\"" + label[0] + "\");"
		elif label[2] == "check":
			text_item += "= gtk_check_menu_item_new_with_label(\"" + label[0] + "\");"
		elif label[2] == "radio":
			group = label[3]
			if radios.keys().count(group) == 0:
				text_item += "= gtk_radio_menu_item_new_with_label(NULL, \"" + label[0] + "\");"
				radios[group] = name_item
			else:
				text_item += "= gtk_radio_menu_item_new_with_label(gtk_radio_menu_item_get_group((GtkRadioMenuItem*)" + radios[group] +  "), \"" + label[0] + "\");"
		else:
			text_item += "= gtk_menu_item_new_with_label(\"" + label[0] + "\");"
